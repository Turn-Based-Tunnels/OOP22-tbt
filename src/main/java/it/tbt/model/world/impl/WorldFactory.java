...
public final class WorldFactory {
    ...
    public static World createWorldDefault() {
        ...
        RoomLink roomLink1 = new RoomLinkImpl("link", CENTER_X, CENTER_Y, STANDARTD_ENTITY_WIDTH, STANDARD_ENTITY_HEIGHT, allyRoom, itemRoom);
        allyRoom.addEntity(roomLink1);
        RoomLink roomLink2 = new RoomLinkImpl("link2", CENTER_X, CENTER_Y, STANDARTD_ENTITY_WIDTH, STANDARD_ENTITY_HEIGHT, itemRoom, shopRoom);
        itemRoom.addEntity(roomLink2);
        RoomLink roomLink3 = new RoomLinkImpl("link3", CENTER_X, CENTER_Y, STANDARTD_ENTITY_WIDTH, STANDARD_ENTITY_HEIGHT, shopRoom, fightRoom);
        shopRoom.addEntity(roomLink3);
        RoomLink roomLink4 = new RoomLinkImpl("link4", CENTER_X, CENTER_Y, STANDARTD_ENTITY_WIDTH, STANDARD_ENTITY_HEIGHT, fightRoom, healRoom);
        fightRoom.addEntity(roomLink4);
        RoomLink roomLink5 = new RoomLinkImpl("link5", CENTER_X, CENTER_Y, STANDARTD_ENTITY_WIDTH, STANDARD_ENTITY_HEIGHT, healRoom, endRoom);
        healRoom.addEntity(roomLink5);
        endRoom.addEntity(new EndEntity("EntityEnd", CENTER_X,CENTER_Y,STANDARTD_ENTITY_WIDTH,STANDARD_ENTITY_HEIGHT, "You finished the game!\nPress any key to quit."));
        Map<Item, Double> drops = new HashMap<>();
        for (final Item item : PotionFactory.getInstance().getItems()) {
            drops.put(item, 0.5);
        }

        fightRoom.addEntity(NPCFactory.createFightNPC("Roberto", TOP_LEFT_X, TOP_LEFT_Y, STANDARD_ENTITY_HEIGHT, STANDARTD_ENTITY_WIDTH, new FightModelImpl(5, drops)));
        ...
        shopItems.put(armors.stream().skip(RND.nextInt(armors.size()-1)).findFirst().get(), 1); // a random weapon
        int wallet = 10000;
        shopRoom.addEntity(NPCFactory.createShopNPC("Merchant", TOP_RIGHT_X, TOP_RIGHT_Y, STANDARD_ENTITY_HEIGHT, STANDARTD_ENTITY_WIDTH, new Shop(shopItems, wallet)));

        //AllyNPC
        List<Skill> skills = new ArrayList<>();
        skills.add((Skill) SkillFactory.getFactory().getSkills().toArray()[0]);
        allyRoom.addEntity(NPCFactory.createAllyNPC ("Ally", BOTTOM_LEFT_X, BOTTOM_LEFT_Y, STANDARD_ENTITY_HEIGHT,STANDARTD_ENTITY_WIDTH, CharacterFactory.createAlly ("Pippo", 50, 50, 50, skills)));

        //Heal
        healRoom.addEntity(NPCFactory.createHealerNPC("Healer", BOTTOM_LEFT_X, BOTTOM_LEFT_Y, STANDARD_ENTITY_HEIGHT,STANDARTD_ENTITY_WIDTH, 10));

        //Item
        Map<Item, Integer> itemsMap = new HashMap<> ();
        itemsMap.put ((Armor) ArmorFactory.getInstance().getItems().toArray()[0], 1);
        itemRoom.addEntity(NPCFactory.createItemNPC("Gifter", BOTTOM_RIGHT_X, BOTTOM_RIGHT_Y, STANDARD_ENTITY_HEIGHT, STANDARTD_ENTITY_WIDTH, itemsMap));
